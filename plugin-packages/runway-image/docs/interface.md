...
- 插件 ID：`runway-image`。
- Provider ID：`runway-image`。
- 能力：`image`。
- 默认 Base URL：`https://api.dev.runwayml.com`。
- 鉴权驱动：`bearer`。
- 创建：`POST /v1/text_to_image`。
...
- `providerOptions.runway-image.body`
- `providerOptions.runway-image.extra_body`
- `providerOptions.runway-image.parameters`
...
该协议的模型级字段变化快或依赖云资源配置。插件固定线协议入口和统一字段，完整厂商对象通过 providerOptions.runway-image 的 parameters/input/extra_body 传入；async Runway image schema。云签名型入口在未配置对应鉴权驱动时会明确失败，不会伪装成 Bearer 成功。
            ...
            "$merge": [
              {
                ...
                "parameters": {
                  "$omitEmpty": {
                    "$ref": "request.providerOptions.runway-image.parameters"
                  }
                }
              },
              {
                "$coalesce": [
                  {
                    "$ref": "request.providerOptions.runway-image.body"
                  },
                  {
                    "$ref": "request.providerOptions.runway-image.extra_body"
                  },
                  {}
                ]
              }
