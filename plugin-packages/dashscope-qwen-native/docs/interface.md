# DashScope Qwen Native 接口字段

## 协议身份

- 插件 ID：`dashscope-qwen-native`。
- Provider ID：`dashscope-qwen-native`。
- 能力：`text`。
- 默认 Base URL：`https://dashscope.aliyuncs.com`。
- 鉴权驱动：`bearer`。
- 创建：`POST /api/v1/services/aigc/text-generation/generation`。
- 生命周期：同步响应。
...
| --- | --- |
| `create.method` | `"POST"` |
| `create.path` | `"/api/v1/services/aigc/text-generation/generation"` |
| `create.contentType` | `"application/json"` |
| `create.body` | `{"$merge":[{"model":{"$ref":"request.model"},"messages":{"$ref":"request.messages"},"input":{"$omitEmpty":{"$ref":"request.prompt"}},"parameters":{"$omitEmpty":{"$ref":"request.providerOptions.dashscope-qwen-native.parameters"}}},{"$coalesce":[{"$ref":"request.providerOptions.dashscope-qwen-native.body"},{"$ref":"request.providerOptions.dashscope-qwen-native.extra_body"},{}]}]}` |
| `create.headers.X-DashScope-Async` | `"enable"` |

## Provider 扩展键

- `providerOptions.dashscope-qwen-native.body`
- `providerOptions.dashscope-qwen-native.extra_body`
- `providerOptions.dashscope-qwen-native.parameters`
...
该协议的模型级字段变化快或依赖云资源配置。插件固定线协议入口和统一字段，完整厂商对象通过 providerOptions.dashscope-qwen-native 的 parameters/input/extra_body 传入；input+parameters。云签名型入口在未配置对应鉴权驱动时会明确失败，不会伪装成 Bearer 成功。
...
```json
{
  ...
  "id": "dashscope-qwen-native",
  "name": "DashScope Qwen Native",
  "version": "2.0.0",
  "author": "Alibaba Cloud / 才遇",
            ...
            "$merge": [
              {
                ...
                "parameters": {
                  "$omitEmpty": {
                    "$ref": "request.providerOptions.dashscope-qwen-native.parameters"
                  }
                }
              },
              {
                "$coalesce": [
                  {
                    "$ref": "request.providerOptions.dashscope-qwen-native.body"
                  },
                  {
                    "$ref": "request.providerOptions.dashscope-qwen-native.extra_body"
                  },
                  {}
                ]
              }
