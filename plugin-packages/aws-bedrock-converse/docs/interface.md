# AWS Bedrock Converse 接口字段

## 协议身份

- 插件 ID：`aws-bedrock-converse`。
- Provider ID：`aws-bedrock-converse`。
- 能力：`text`。
- 默认 Base URL：`https://bedrock-runtime.{region}.amazonaws.com`。
- 鉴权驱动：`aws-sigv4`。
- 创建：`POST /model/{{model}}/converse`。
- 生命周期：同步响应。
...
| 字段 | 类型 | 必填 | 含义 |
| --- | --- | --- | --- |
| `apiKey` | secret | 是 | Access Key ID |
| `secretKey` | secret | 是 | Secret Access Key |
...
| --- | --- |
| `create.method` | `"POST"` |
| `create.path` | `"/model/{{model}}/converse"` |
| `create.contentType` | `"application/json"` |
| `create.body` | `{"$merge":[{"model":{"$ref":"request.model"},"messages":{"$ref":"request.messages"},"input":{"$omitEmpty":{"$ref":"request.prompt"}},"parameters":{"$omitEmpty":{"$ref":"request.providerOptions.aws-bedrock-converse.parameters"}}},{"$coalesce":[{"$ref":"request.providerOptions.aws-bedrock-converse.body"},{"$ref":"request.providerOptions.aws-bedrock-converse.extra_body"},{}]}]}` |

## Provider 扩展键

- `providerOptions.aws-bedrock-converse.body`
- `providerOptions.aws-bedrock-converse.extra_body`
- `providerOptions.aws-bedrock-converse.parameters`
...
该协议的模型级字段变化快或依赖云资源配置。插件固定线协议入口和统一字段，完整厂商对象通过 providerOptions.aws-bedrock-converse 的 parameters/input/extra_body 传入；messages+system+inferenceConfig+toolConfig。云签名型入口在未配置对应鉴权驱动时会明确失败，不会伪装成 Bearer 成功。
            ...
            "$merge": [
              {
                ...
                "parameters": {
                  "$omitEmpty": {
                    "$ref": "request.providerOptions.aws-bedrock-converse.parameters"
                  }
                }
              },
              {
                "$coalesce": [
                  {
                    "$ref": "request.providerOptions.aws-bedrock-converse.body"
                  },
                  {
                    "$ref": "request.providerOptions.aws-bedrock-converse.extra_body"
                  },
                  {}
                ]
              }
