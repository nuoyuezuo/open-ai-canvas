# MiniMax Text Native 接口字段

## 协议身份

- 插件 ID：`minimax-text-native`。
- Provider ID：`minimax-text-native`。
- 能力：`text`。
- 默认 Base URL：`https://api.minimax.chat`。
- 鉴权驱动：`bearer`。
- 创建：`POST /v1/text/chatcompletion_v2`。
- 生命周期：同步响应。
...
| --- | --- |
| `create.method` | `"POST"` |
| `create.path` | `"/v1/text/chatcompletion_v2"` |
| `create.contentType` | `"application/json"` |
| `create.body` | `{"$merge":[{"model":{"$ref":"request.model"},"messages":{"$ref":"request.messages"},"input":{"$omitEmpty":{"$ref":"request.prompt"}},"parameters":{"$omitEmpty":{"$ref":"request.providerOptions.minimax-text-native.parameters"}}},{"$coalesce":[{"$ref":"request.providerOptions.minimax-text-native.body"},{"$ref":"request.providerOptions.minimax-text-native.extra_body"},{}]}]}` |

## Provider 扩展键

- `providerOptions.minimax-text-native.body`
- `providerOptions.minimax-text-native.extra_body`
- `providerOptions.minimax-text-native.parameters`
...
该协议的模型级字段变化快或依赖云资源配置。插件固定线协议入口和统一字段，完整厂商对象通过 providerOptions.minimax-text-native 的 parameters/input/extra_body 传入；messages+tokens_to_generate。云签名型入口在未配置对应鉴权驱动时会明确失败，不会伪装成 Bearer 成功。
            ...
            "$merge": [
              {
                ...
                "parameters": {
                  "$omitEmpty": {
                    "$ref": "request.providerOptions.minimax-text-native.parameters"
                  }
                }
              },
              {
                "$coalesce": [
                  {
                    "$ref": "request.providerOptions.minimax-text-native.body"
                  },
                  {
                    "$ref": "request.providerOptions.minimax-text-native.extra_body"
                  },
                  {}
                ]
              }
