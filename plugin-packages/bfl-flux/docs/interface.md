# Black Forest Labs FLUX 接口字段

## 协议身份

- 插件 ID：`bfl-flux`。
- Provider ID：`bfl-flux`。
- 能力：`image`。
- 默认 Base URL：`https://api.bfl.ai`。
- 鉴权驱动：`bearer`。
- 创建：`POST /v1/{{model}}`。
- 查询：`GET /v1/get_result`。
...
| --- | --- |
| `create.method` | `"POST"` |
| `create.path` | `"/v1/{{model}}"` |
| `create.contentType` | `"application/json"` |
| `create.body` | `{"$merge":[{"model":{"$ref":"request.model"},"prompt":{"$ref":"request.prompt"},"images":{"$omitEmpty":{"$map":{"from":{"$ref":"request.images"},"as":"media","in":{"$ref":"media.value"}}}},"parameters":{"$omitEmpty":{"$ref":"request.providerOptions.bfl-flux.parameters"}}},{"$coalesce":[{"$ref":"request.providerOptions.bfl-flux.body"},{"$ref":"request.providerOptions.bfl-flux.extra_body"},{}]}]}` |
| `poll.method` | `"GET"` |
| `poll.path` | `"/v1/get_result"` |
| `poll.contentType` | `"application/json"` |

## Provider 扩展键

- `providerOptions.bfl-flux.body`
- `providerOptions.bfl-flux.extra_body`
- `providerOptions.bfl-flux.parameters`
...
该协议的模型级字段变化快或依赖云资源配置。插件固定线协议入口和统一字段，完整厂商对象通过 providerOptions.bfl-flux 的 parameters/input/extra_body 传入；async model schema。云签名型入口在未配置对应鉴权驱动时会明确失败，不会伪装成 Bearer 成功。
            ...
            "$merge": [
              {
                ...
                "parameters": {
                  "$omitEmpty": {
                    "$ref": "request.providerOptions.bfl-flux.parameters"
                  }
                }
              },
              {
                "$coalesce": [
                  {
                    "$ref": "request.providerOptions.bfl-flux.body"
                  },
                  {
                    "$ref": "request.providerOptions.bfl-flux.extra_body"
                  },
                  {}
                ]
              }
