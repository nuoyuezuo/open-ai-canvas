...
- 插件 ID：`kling-image`。
- Provider ID：`kling-image`。
- 能力：`image`。
- 默认 Base URL：`https://api.klingai.com`。
- 鉴权驱动：`bearer`。
- 创建：`POST /v1/images/generations`。
- 查询：`GET /v1/images/generations/{{taskId}}`。
...
| `create.body` | `{"$merge":[{"model":{"$ref":"request.model"},"prompt":{"$ref":"request.prompt"},"images":{"$omitEmpty":{"$map":{"from":{"$ref":"request.images"},"as":"media","in":{"$ref":"media.value"}}}},"parameters":{"$omitEmpty":{"$ref":"request.providerOptions.kling-image.parameters"}}},{"$coalesce":[{"$ref":"request.providerOptions.kling-image.body"},{"$ref":"request.providerOptions.kling-image.extra_body"},{}]}]}` |
| `poll.method` | `"GET"` |
| `poll.path` | `"/v1/images/generations/{{taskId}}"` |
| `poll.contentType` | `"application/json"` |

## Provider 扩展键

- `providerOptions.kling-image.body`
- `providerOptions.kling-image.extra_body`
- `providerOptions.kling-image.parameters`
...
该协议的模型级字段变化快或依赖云资源配置。插件固定线协议入口和统一字段，完整厂商对象通过 providerOptions.kling-image 的 parameters/input/extra_body 传入；task JSON。云签名型入口在未配置对应鉴权驱动时会明确失败，不会伪装成 Bearer 成功。
            ...
            "$merge": [
              {
                ...
                "parameters": {
                  "$omitEmpty": {
                    "$ref": "request.providerOptions.kling-image.parameters"
                  }
                }
              },
              {
                "$coalesce": [
                  {
                    "$ref": "request.providerOptions.kling-image.body"
                  },
                  {
                    "$ref": "request.providerOptions.kling-image.extra_body"
                  },
                  {}
                ]
              }
