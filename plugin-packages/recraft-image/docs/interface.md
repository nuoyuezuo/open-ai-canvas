...
- 插件 ID：`recraft-image`。
- Provider ID：`recraft-image`。
- 能力：`image`。
- 默认 Base URL：`https://external.api.recraft.ai`。
- 鉴权驱动：`bearer`。
...
| `create.body` | `{"$merge":[{"model":{"$ref":"request.model"},"prompt":{"$ref":"request.prompt"},"images":{"$omitEmpty":{"$map":{"from":{"$ref":"request.images"},"as":"media","in":{"$ref":"media.value"}}}},"parameters":{"$omitEmpty":{"$ref":"request.providerOptions.recraft-image.parameters"}}},{"$coalesce":[{"$ref":"request.providerOptions.recraft-image.body"},{"$ref":"request.providerOptions.recraft-image.extra_body"},{}]}]}` |

## Provider 扩展键

- `providerOptions.recraft-image.body`
- `providerOptions.recraft-image.extra_body`
- `providerOptions.recraft-image.parameters`
...
该协议的模型级字段变化快或依赖云资源配置。插件固定线协议入口和统一字段，完整厂商对象通过 providerOptions.recraft-image 的 parameters/input/extra_body 传入；raster/vector image JSON。云签名型入口在未配置对应鉴权驱动时会明确失败，不会伪装成 Bearer 成功。
            ...
            "$merge": [
              {
                ...
                "parameters": {
                  "$omitEmpty": {
                    "$ref": "request.providerOptions.recraft-image.parameters"
                  }
                }
              },
              {
                "$coalesce": [
                  {
                    "$ref": "request.providerOptions.recraft-image.body"
                  },
                  {
                    "$ref": "request.providerOptions.recraft-image.extra_body"
                  },
                  {}
                ]
              }
