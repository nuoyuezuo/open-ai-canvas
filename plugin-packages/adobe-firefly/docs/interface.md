...
- 能力：`image`。
- 默认 Base URL：`https://firefly-api.adobe.io`。
- 鉴权驱动：`bearer`。
- 创建：`POST /v3/images/generate`。
- 生命周期：同步响应。
...
| --- | --- |
| `create.method` | `"POST"` |
| `create.path` | `"/v3/images/generate"` |
| `create.contentType` | `"application/json"` |
| `create.body` | `{"$merge":[{"model":{"$ref":"request.model"},"prompt":{"$ref":"request.prompt"},"images":{"$omitEmpty":{"$map":{"from":{"$ref":"request.images"},"as":"media","in":{"$ref":"media.value"}}}},"parameters":{"$omitEmpty":{"$ref":"request.providerOptions.adobe-firefly.parameters"}}},{"$coalesce":[{"$ref":"request.providerOptions.adobe-firefly.body"},{"$ref":"request.providerOptions.adobe-firefly.extra_body"},{}]}]}` |

## Provider 扩展键

- `providerOptions.adobe-firefly.body`
- `providerOptions.adobe-firefly.extra_body`
- `providerOptions.adobe-firefly.parameters`
...
该协议的模型级字段变化快或依赖云资源配置。插件固定线协议入口和统一字段，完整厂商对象通过 providerOptions.adobe-firefly 的 parameters/input/extra_body 传入；versioned Firefly request。云签名型入口在未配置对应鉴权驱动时会明确失败，不会伪装成 Bearer 成功。
            ...
            "$merge": [
              {
                ...
                "parameters": {
                  "$omitEmpty": {
                    "$ref": "request.providerOptions.adobe-firefly.parameters"
                  }
                }
              },
              {
                "$coalesce": [
                  {
                    "$ref": "request.providerOptions.adobe-firefly.body"
                  },
                  {
                    "$ref": "request.providerOptions.adobe-firefly.extra_body"
                  },
                  {}
                ]
              }
