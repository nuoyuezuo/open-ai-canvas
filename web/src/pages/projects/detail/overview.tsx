...
export default function ProjectOverviewView({ detail, overview }: ProjectDetailViewProps & { overview: ProjectOverview }) {
    ...
    const profile = useProjectWorkflowProfile();
    // 漫画把「分格」和「成稿画面」拆开，概览沿用同一份聚合数据但换一套阶段文案。
    const isComic = profile.id === "comic";
    const panelLabel = profile.unitItemLabel;
    ...
    const workflowHref = (targetStage: string) => firstUnitId ? projectRoute(routeBase, project.id, "workflow", firstUnitId, targetStage) : projectRoute(routeBase, project.id, "chapters");
    const stage = overviewStage(metrics, isComic, panelLabel);
    const actions = overviewActions(routeBase, project.id, metrics, firstUnitId, isComic, panelLabel).slice(0, 3);
    ...
    const unitStages = overview.units.map((item) => ({
        ...
        storyboard: stageCell(item.shotCount > 0, item.shotCount ? `${item.shotCount} ${isComic ? "格" : "镜"}` : "待生成"),
        canvas: stageCell(item.canvasCount > 0, item.canvasCount ? `${item.canvasCount} 张` : "未关联"),
    }));
    const productionSteps = isComic
        ? [
            { id: "story", icon: BookOpenText, label: "原著与改编", description: "导入原作、检索改编或直接编写本章正文", metric: `${metrics.unitCount} 章 · ${formatCompactCount(metrics.totalWordCount)} 字`, href: projectRoute(routeBase, project.id, "chapters"), complete: metrics.unitCount > 0 },
            { id: "assets", icon: UsersRound, label: "角色与场景", description: "确认角色、场景、道具和漫画画风", metric: `${metrics.assetCount} 项资产`, href: projectRoute(routeBase, project.id, "assets"), complete: metrics.assetCount > 0 },
            { id: "panels", icon: Clapperboard, label: "分格脚本", description: "拆分画格并确认版面、对白和叙事节拍", metric: `${metrics.shotCount} 格`, href: workflowHref("panels"), complete: metrics.shotCount > 0 },
            { id: "inking", icon: Film, label: "成稿画面", description: "逐格生成黑白成稿并锁定版本", metric: `${metrics.readyStoryboardCount}/${metrics.shotCount || 0} 格`, href: workflowHref("inking"), complete: metrics.shotCount > 0 && metrics.readyStoryboardCount === metrics.shotCount },
            { id: "delivery", icon: PackageCheck, label: "交付与导出", description: "检查缺失画格并整理最终产物", metric: metrics.readyStoryboardCount && metrics.readyStoryboardCount === metrics.shotCount ? "可以交付" : `还差 ${Math.max(0, metrics.shotCount - metrics.readyStoryboardCount)} 格`, href: workflowHref("delivery"), complete: metrics.shotCount > 0 && metrics.readyStoryboardCount === metrics.shotCount },
        ]
        : [
            { id: "story", icon: BookOpenText, label: "剧情章节", description: "导入或编写正文，确认每章叙事目标", metric: `${metrics.unitCount} 章 · ${formatCompactCount(metrics.totalWordCount)} 字`, href: projectRoute(routeBase, project.id, "chapters"), complete: metrics.unitCount > 0 },
            { id: "assets", icon: UsersRound, label: "角色与资产", description: "确认角色、场景、道具和项目画风", metric: `${metrics.assetCount} 项资产`, href: projectRoute(routeBase, project.id, "assets"), complete: metrics.assetCount > 0 },
    ...
    const gaps = [
        ...
        isComic
            ? metrics.shotCount ? metrics.readyStoryboardCount === metrics.shotCount ? "所有画格成稿已生成" : `${metrics.shotCount - metrics.readyStoryboardCount} 个画格尚未生成成稿` : `还没有${panelLabel}`
    ...
    return (
        <div className="space-y-8">
            ...
            <section className="project-standard-flow">
                <div className="project-standard-flow-head"><div><span>标准制作流程</span><h2>{isComic ? "从原著到可交付画格" : "从章节到可交付镜头"}</h2><p>{isComic ? "先确认原著改编与资产，再逐格完成分格脚本和成稿画面。每个步骤都可直接进入对应工作区。" : "先确认故事与资产，再逐镜完成画面、动作和视频。每个步骤都可直接进入对应工作区。"}</p></div><Link to={primaryAction.href}>继续当前任务<ArrowRight /></Link></div>
                ...
                <div className="project-standard-flow-footer"><div><strong>当前制作检查</strong>{gaps.map((gap, index) => <span key={gap}><i className={index === 2 && (isComic ? metrics.readyStoryboardCount !== metrics.shotCount : metrics.readyVideoCount !== metrics.shotCount) ? "is-attention" : ""} />{gap}</span>)}</div><div><strong>快速入口</strong><Link to={projectRoute(routeBase, project.id, "chapters")}>整理章节</Link><Link to={projectRoute(routeBase, project.id, "assets")}>确认资产</Link><Link to={workflowHref(isComic ? "inking" : "video")}>{isComic ? "继续画格制作" : "继续镜头制作"}</Link></div></div>
            </section>
            ...
            <section>
                <div className="project-pipeline-head">
                    ...
                    <Link to={projectRoute(routeBase, project.id, "chapters")} className="project-pipeline-more">查看全部章节<ArrowRight className="size-3.5" /></Link>
                </div>

                {unitStages.length ? (
                    <div className="project-pipeline-surface">
                        {unitStages.map((item) => (
                            <Link key={item.unit.id} to={projectRoute(routeBase, project.id, "chapters", item.unit.id)} className="project-pipeline-row group">
                                ...
                                <StagePipeline content={item.content} assets={item.assets} storyboard={item.storyboard} canvas={item.canvas} stageLabel={isComic ? "分格" : "分镜"} />
                                <ArrowRight className="project-pipeline-arrow size-4" />
                            </Link>
                        ))}
                    </div>
                ) : <div className="project-pipeline-surface p-2"><WorkspaceState icon="projects" compact title="还没有剧情章节" description={`添加章节后，这里会显示内容、资产、${isComic ? "分格" : "分镜"}和画布的制作进度。`} /></div>}
            </section>
        </div>
    );
}
...
function StagePipeline({ content, assets, storyboard, canvas, stageLabel }: { content: ProjectStageCell; assets: ProjectStageCell; storyboard: ProjectStageCell; canvas: ProjectStageCell; stageLabel: string }) {
    const stages = [{ label: "内容", cell: content }, { label: "资产", cell: assets }, { label: stageLabel, cell: storyboard }, { label: "画布", cell: canvas }];
    return (
...
function overviewStage(metrics: ProjectOverviewMetrics, isComic: boolean, panelLabel: string) {
    ...
    if (!metrics.shotCount || metrics.unitsWithoutShots) return { label: isComic ? "分格准备" : "分镜准备", detail: `${metrics.unitsWithoutShots || metrics.unitCount} 章待生成${isComic ? "分格脚本" : "分镜"}` };
    if (isComic) {
        if (metrics.readyStoryboardCount < metrics.shotCount) return { label: "画格制作", detail: `${metrics.readyStoryboardCount}/${metrics.shotCount} 格成稿就绪` };
        return { label: "检查交付", detail: "画格成稿已准备完成" };
    }
...
function overviewActions(routeBase: string, projectId: string, metrics: ProjectOverviewMetrics, firstUnitId: string | undefined, isComic: boolean, panelLabel: string): ProjectWorkbenchAction[] {
    const projectRoot = projectRoute(routeBase, projectId);
    const workflowHref = firstUnitId ? projectRoute(routeBase, projectId, "workflow", firstUnitId, isComic ? "inking" : "video") : projectRoute(routeBase, projectId, "chapters");
    const readyCount = isComic ? metrics.readyStoryboardCount : metrics.readyVideoCount;
    const readyNoun = isComic ? "成稿" : "视频";
    ...
    if (!metrics.shotCount || metrics.unitsWithoutShots) {
        actions.push({ id: "create-storyboards", title: `为 ${metrics.unitsWithoutShots || metrics.unitCount} 章建立${isComic ? "分格脚本" : "分镜"}`, description: isComic ? "按章节生成画格草稿，再逐格调整版面、对白和叙事节拍。" : "按章节生成镜头草稿，再逐镜调整画面、对白和时长。", href: firstUnitId ? projectRoute(routeBase, projectId, "chapters", firstUnitId) : `${projectRoot}/chapters`, actionLabel: isComic ? "建立分格" : "建立分镜", tone: "default" });
    }
    if (metrics.shotCount && readyCount < metrics.shotCount) {
        actions.push({ id: "continue-production", title: `继续制作 ${metrics.shotCount - readyCount} 个${isComic ? "画格" : "镜头"}${readyNoun}`, description: isComic ? "检查画格提示词与参考资产，逐格生成成稿并选择最终版本。" : "检查镜头提示词与参考资产，逐镜生成并选择最终版本。", href: workflowHref, actionLabel: "继续制作", tone: metrics.staleArtifactCount ? "attention" : "default" });
    }
    ...
    if (!actions.length) {
        actions.push({ id: "review-delivery", title: `检查${isComic ? panelLabel : "镜头"}并准备交付`, description: isComic ? "所有画格成稿已就绪，可检查版本、连续性和缺失项。" : "所有镜头视频已就绪，可检查版本、连续性和缺失项。", href: workflowHref, actionLabel: "检查交付", tone: "default" });
    }
    return actions;
}
